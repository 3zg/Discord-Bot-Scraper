...
@bot.event
async def on_ready():
    member = bot.get_user(config["memberid"])
    print(f"""
    ...
    if choice == "1":
        ...
        print(f"""
         ...
         """)
        if len(bot.guilds) == 0: 
            print(F"{Fore.GREEN}[!] {Fore.RESET}{Fore.RED} This bot is not in any guilds {Fore.RESET}")
        else:      
            ...
            if not (int(guild) in bot.guilds):
                ...
                print(F"{Fore.GREEN} [!] {Fore.RESET}{Fore.RED} This bot is not in this guild or its not a guild. {Fore.RESET}")
        ...
        if len(bot.guilds) == 0: 
            ...
            messageto = await bot.fetch_user(config["memberid"])
